 ...
 * \ingroup    einvoicing
 * \brief      Render an XML source as coloured, foldable, numbered HTML - what a browser shows for an
 *             XML document it displays itself.
 *
 * Deliberately self-contained: it depends on no class, no global and no other function of Dolibarr, so
 * the same two functions can be moved to the core the day the native preview learns to handle XML.
 * Nothing here parses the document: the source is tokenised as text and printed escaped, so a truncated
 * or invalid file is still displayed as it is on disk instead of producing nothing.
 */
