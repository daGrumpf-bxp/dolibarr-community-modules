 ...
 *      \ingroup    test
 *      \brief      PHPUnit test for the coloured XML source of the read-only viewer.
 *                  The viewer exists because the core serves an XML as application/octet-stream on
 *                  purpose: an inline text/xml response from the Dolibarr origin can carry an
 *                  xml-stylesheet processing instruction and have the browser render arbitrary HTML
 *                  in that origin. Colouring the source means writing markup around it, so the two
 *                  properties that make it safe are the ones tested here: nothing of the document
 *                  survives as markup, and the file still reads exactly as it is on disk.
