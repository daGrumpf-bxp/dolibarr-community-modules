...
class SuperPDPProvider extends AbstractPDPProvider
{
	...
	public function callApi($resource, $method, $params = false, $extraHeaders = [], $callType = '')
	{
		...
		// validation_reports lives on the AP API base; the AFNOR directory (annuaire) lives on the
		// afnor-directory/v1 base, distinct from the Flow API base (afnor-flow/v1, same host different path).
		// Route each call to the right base.
		if ($resource == 'token' || $callType == 'get_access_token') {
			$url = $this->getApiUrl('auth') . $resource;
		} elseif ($resource == 'validation_reports') {
